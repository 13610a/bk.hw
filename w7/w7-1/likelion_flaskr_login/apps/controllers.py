...
#
#@before request is always did before every functions
#
@app.before_request
def before_request():
    g.user_name = None
...
def article_create():
	...
	if request.method == 'POST':
		if form.validate_on_submit():
			...
			flash(u'글이 작성됐다능 나중에 지우지나 말라능.', 'success')
...
def article_delete(id):
	if request.method == 'GET':
		...
		flash(u'게시글을 지웠다..하지만 난 니가 또 쓸거란걸 알지.', 'success')
		return redirect(url_for('article_list'))
...
def comment_delete(id):
	if request.method == 'POST':
		...
		if request.form['password'] == comment.password:
			...
			flash(u'댓글삭제했다능.', 'success')
			return redirect(url_for('article_detail', id=article_id))
		else:
			flash(u'비번도 틀릴거면서 삭제를 도모했단 말이야?.', 'danger')
			return render_template('comment/delete.html', comment_id=request.form['comment_id'])

	elif request.method == 'GET':
		flash(u'경고! 너의 소중한 댓글이 진짜 지워진다능?.', 'warning')
	   ...
	   if form.validate_on_submit():
			...
			try: 
				...
				if not check_password_hash(user.password, password): 
					flash(u'이멜,비번 확인바람. 넌 우리쪽 사람이 아닌거같아.', 'danger') 
